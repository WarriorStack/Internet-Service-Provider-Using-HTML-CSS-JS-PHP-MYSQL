...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['profilePhoto']) && $_FILES['profilePhoto']['error'] === 0) {
        $targetDir = $_SERVER['DOCUMENT_ROOT'] . "/ISPProject/uploads/"; // Direct path to /ISPProject/uploads/
        ...
        if (move_uploaded_file($_FILES['profilePhoto']['tmp_name'], $targetFile)) {
            chmod($targetFile, 0644); // Ensure proper permissions
            ...
            $stmt->execute(["/ISPProject/uploads/" . $fileName, $cid]);
        
            $successMessage = 'Profile photo updated successfully!';
        } else {
            $successMessage = 'Failed to upload profile photo.';
        }
